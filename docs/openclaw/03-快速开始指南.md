...
> **2026-04 差量更新**
>
> - 状态检查优先用 `openclaw gateway status`，`openclaw status` 可视作更高层概览入口
> - 当前官方 CLI 同时保留 `openclaw health`、`openclaw doctor`、`openclaw docs`
> - 后续进阶阅读里，不再把“29 个模型 / 50+ 技能”这类容易变化的数字写成固定承诺
