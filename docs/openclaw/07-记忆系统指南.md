...
记忆系统真正好用，不是因为它“什么都记得”，而是因为它知道什么应该长期保留，什么只应该进入当天日志，什么应该完全不写。下面这次练习会把 `USER.md`、`MEMORY.md`、每日记忆和索引命令串起来。
...
如果不是要彻底删除，而是要让旧信息不再主导回答，可以在 `MEMORY.md` 写清楚新的事实：
...
记忆冲突不是越删越好。更好的整理方式，是把“默认规则”“例外情况”“适用场景”写清楚。
