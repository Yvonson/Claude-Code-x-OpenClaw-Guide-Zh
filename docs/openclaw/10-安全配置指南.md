...
> **v2026.6.1 安全更新**：`models status` 会保留 SecretRef-backed custom provider 的 `apiKey` 标记，避免把解析后的明文 secret 写回 `models.json`。`doctor` 也会提示明文 secret-bearing config fields。v2026.6.1 继续强化 corrupt shell snapshot、suspicious gateway startup config、unsafe exec precheck env、oversized audit responses、插件 disabled snapshot 和 invalid pending-agent SQLite scaffold 等边界。团队配置里优先使用 SecretRef、环境变量或外部密钥服务，不要把 provider API key 和敏感 headers 写成普通 JSON 字段。
...
> 建议：订阅 OpenClaw 的 GitHub Security Advisories，第一时间获取安全更新通知。v2026.5.22 已包含 `protobufjs` 8.4.0 安全更新；v2026.6.1 又补充多项运行时、插件、Gateway 和审计边界。如果你用 Docker 或固定 npm tag，升级后要重新构建镜像并保留 lockfile / shrinkwrap 完整性校验。
